...
type LWControl() =
  ...
  default this.HitTest p =
    (new RectangleF(PointF(0.f,0.f), size)).Contains(p)
...
type LWContainer() as this =
  ...
  override this.OnMouseDown e =
    ...
    match (controlsView |> Seq.tryFind (fun c -> c.HitTest (PointF(p.X - c.Position.X,p.Y - c.Position.Y)) )) with
    ...
    | None -> 
      ...
      match (controlsWorld |> Seq.tryFind(fun c -> c.HitTest (PointF(pw.X - c.Position.X,pw.Y - c.Position.Y)))) with
      | Some c -> c.OnMouseDown(e)
      | None -> ()
  ...
  override this.OnPaint e =
    ...
    //viene effettuata una traslazione in modo tale che l'origine del contesto grafico corrente sia fissata a position
    //viene delimitata la regione di disegno del LWControl 
    //viene creato un nuovo evento paint che interessa solo questa regione.
    let TranslateClip (c:LWControl) =
        let s = e.Graphics.Save()
        e.Graphics.TranslateTransform(c.Position.X, c.Position.Y)
